...
log₂n 就是n里面有几个2的意思， log₂(8) = 3， N==8, 那么每层二分，可以分3层
...
int partition(vector<int>& nums, int left, int right) {
    // 步骤1：选择左端元素作为pivot（也可选中间/随机，避免最坏情况）
    int pivot = nums[left];
    int i = left;   // 左指针（从左向右找 > pivot 的元素）
    int j = right;  // 右指针（从右向左找 < pivot 的元素）

    while (i < j) {
        // 步骤2：右指针向左移动，直到找到 < pivot 的元素（跳过 ≥ pivot 的元素）
        while (i < j && nums[j] >= pivot) {
            j--;
        }
        // 步骤3：左指针向右移动，直到找到 > pivot 的元素（跳过 ≤ pivot 的元素）
        while (i < j && nums[i] <= pivot) {
            i++;
        }
        // 步骤4：交换左右指针指向的元素（此时 nums[i] > pivot，nums[j] < pivot）
        if (i < j) {
            swap(nums[i], nums[j]);
        }
    }

    // 步骤5：将pivot交换到最终位置（i=j处，此时左边≤pivot，右边≥pivot）
    swap(nums[left], nums[i]);
    return i;  // 返回pivot的索引，用于递归分治
}

// 快速排序主函数（递归）
void quickSort(vector<int>& nums, int left, int right) {
    if (left < right) {
        int pivotIdx = partition(nums, left, right);  // 分区
...
def partition(nums, left, right):
    ...
    while i < j:
        # 步骤2：右指针向左找 < pivot 的元素
        while i < j and nums[j] >= pivot:
            j -= 1
        # 步骤3：左指针向右找 > pivot 的元素
...
```
def partition_optimized(nums, left, right):
    # 步骤1：三数取中法选择pivot（避免最坏情况）
    mid = left + (right - left) // 2  # 中间索引（防止溢出）
    # 排序左、中、右三个元素，取中位数作为pivot
    ...
    # 此时 nums[mid] 是中位数，将其交换到左端作为pivot
    nums[left], nums[mid] = nums[mid], nums[left]
    pivot = nums[left]
