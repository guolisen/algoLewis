    ...
    def zigzagLevelOrder(self, root: Optional[TreeNode]) -> List[List[int]]:
        ...
        while len(q) > 0:
            ...
            for _ in range(level):
                cur = q.popleft()
                if step % 2 == 0:
                    curorder.append(cur.val)
                else:
                    curorder.insert(0, cur.val)
                if cur.left:
                    q.append(cur.left)
                if cur.right:
                    q.append(cur.right)
...
class Solution:
    def verticalTraversal(self, root: Optional[TreeNode]) -> List[List[int]]:
        ...
        while len(q) > 0:
            ...
            h[col].append((row, cur.val))
            if cur.left:
                q.append((cur.left, row + 1, col - 1))
            if cur.right:
                q.append((cur.right, row + 1, col + 1))
        res = []
        for _, v in sorted(h.items(), key = lambda x : x[0]):  
            curlist = [v for k, v in sorted(v, key = lambda x : (x[0], x[1]))]
            res.append(curlist)
        return res
```
...
class Solution:
    def search(self, root, depth):
        if root == None:
            return depth
        l = self.search(root.left, depth + 1)
        r = self.search(root.right, depth + 1)
        if abs(r - l) > 1:
            self.res = False
        return max(l, r)
...
#### 5. 判断完全二叉树 → **LeetCode 958. Check Completeness of a Binary Tree**
   ...
   宽搜, 层序遍历，如果有left,right是null 就标记flag = true, 后面再遇到不空的就认为不是完全二叉树
...
#### 2. 路径总和 II → **LeetCode 113. Path Sum II**
   ...
   class Solution {
   public:
       void dfs(TreeNode* root, int target, int s, vector<int>& curlist, vector<vector<int>>& res)
       {
           if (root == nullptr)
               return;
           ...
           curlist.push_back(root->val);
           dfs(root->left, target, s + root->val, curlist, res);
           dfs(root->right, target, s + root->val, curlist, res);
           curlist.pop_back();
       }
       vector<vector<int>> pathSum(TreeNode* root, int targetSum) {
           ...
           vector<vector<int>> res;
           vector<int> curlist;
           dfs(root, targetSum, 0, curlist, res);
           return res;
       }
   };
   ```
...
#### 3. 最大二叉树 → **LeetCode 654. Maximum Binary Tree**
 ...
 public:
     TreeNode* dfs(vector<int>& nums, int l, int r)
     {
         ...
         auto mx = max_element(nums.begin() + l, nums.begin() + r + 1);
         for (int i = l; i <= r; i++)
         {
             if (nums[i] != *mx)
                 continue;
             auto current = new TreeNode(nums[i]);
             current->left = dfs(nums, l, i - 1);
             current->right = dfs(nums, i + 1, r);
             return current;
         }
         return nullptr;
     }
...
#### 6. 修剪二叉搜索树 → **LeetCode 669. Trim a Binary Search Tree**  <<<
 ...
 class Solution:
     def dfs(self, root, low, high):
         ...
         current.left = self.dfs(root.left, low, root.val)
         current.right = self.dfs(root.right, root.val, high)
         return current
