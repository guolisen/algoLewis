...
class Solution:
    def isIsomorphic(self, s: str, t: str) -> bool:
        shash = collections.defaultdict(str)
        thash = collections.defaultdict(str)
        m = len(s)
        for i in range(m):
            if s[i] in shash.keys():
                if shash[s[i]] != t[i]:
                    return False
            if t[i] in thash.keys():
                if thash[t[i]] != s[i]:
                    return False
            shash[s[i]] = t[i]
            thash[t[i]] = s[i]
...
```
   ...
   class Solution:
       def wordPattern(self, pattern: str, s: str) -> bool:
           ...
           m = len(pattern)
           for i in range(m):
               if pattern[i] in phash.keys():
                   if phash[pattern[i]] != words[i]:
                       return False
               if words[i] in shash.keys():
                   if shash[words[i]] != pattern[i]:
                       return False
...
class Solution:
    def frequencySort(self, s: str) -> str:
        ...
        res = ""
        for k, v in sorted(h.items(), key = lambda x : (-x[1], x[0])):
            for _ in range(v):
                res += k
        return res
```
...
public:
    typedef unsigned long long ULL;
    const ULL MOD = 1e9 + 7;  // 大素数
    
    ULL getHash(vector<ULL>& h, vector<ULL>& p, int L, int R)
    {
        // 注意：这里需要处理负数情况，使用 (h[R] - h[L-1]*p[R-L+1] + MOD) % MOD
    ...
    string longestPrefix(string s) {
        ...
        vector<ULL> shash(m + 1, 0);
        vector<ULL> p(m + 1, 0);
        const ULL P = 131;  // 常用大基数
        
        p[0] = 1;
        for (int i = 1; i <= m; i++)
        {
            p[i] = p[i - 1] * P % MOD;
            shash[i] = (shash[i - 1] * P % MOD + s[i - 1]) % MOD;
        }
        
        for (int len = m - 1; len >= 1; len--)
        {
            ULL lh = getHash(shash, p, 1, len);
            ULL rh = getHash(shash, p, m - len + 1, m);
            if (lh == rh)
...
```
数学原理
在模运算中，我们需要保证结果是 [0, MOD-1] 范围内的正数。

对于 (a - b) % MOD，如果 a < b，结果会是负数。

解决方法：

text
(a - b) % MOD = (a - b + MOD) % MOD
因为 MOD % MOD = 0，所以加上 MOD 不会改变模运算的结果，但可以保证括号内的值非负。
```
...
public:
    ...
    string longestPrefix(string s) {
        ...
        {
            auto prefix = getHash(hs, p, 1, len);
            auto subfix = getHash(hs, p, m - len + 1, m);
            if (prefix == subfix)
                return s.substr(0, len);
        }
        return "";
...
class Solution:
    def longestDiverseString(self, a: int, b: int, c: int) -> str:
        ...
        res = ""
        while len(q) > 0:
            curCount, curChar = heapq.heappop(q)
            if len(res) >= 2 and res[-1] == curChar and res[-2] == curChar:
                if len(q) == 0:
                    break
                nextCount, nextChar = heapq.heappop(q)
                res += nextChar
                nextCount = -nextCount
                nextCount -= 1
                if nextCount > 0:
                    heapq.heappush(q, (-nextCount, nextChar))
                heapq.heappush(q, (curCount, curChar))
            else:
                res += curChar
                curCount = -curCount
                curCount -= 1
...
![](./assets/屏幕截图 2026-01-01 151318.png)
...
public:
    int myAtoi(string s) {
        ...
        int res = 0;
        while (idx < m && isdigit(s[idx]))
        {
            int single = s[idx] - '0';
            if (flag > 0 && (res > INT_MAX / 10 || (res == INT_MAX / 10 && single >= 7)))
                return INT_MAX;
            if (flag < 0 && (-res < INT_MIN / 10 || (-res == INT_MIN / 10 && -single <= -8)))
                return INT_MIN; 
            res = res * 10 + single;
            idx++;
        }
...
class Solution {
public:
    bool isLeftNear(vector<int>& a, int l, int r, int x)
    {
        int m = a.size();
        if (l < 0)
            return false;
    ...
    vector<int> findClosestElements(vector<int>& arr, int k, int x) {
        ...
        while (res.size() < k)
        {
            if (isLeftNear(arr, ll, rr, x))
            {
                res.push_back(arr[ll]);
                ll--;
            }
            else
            {
                res.push_back(arr[rr]);
                rr++;
            }
        }
        sort(res.begin(), res.end());
