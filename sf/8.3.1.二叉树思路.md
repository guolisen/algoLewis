...
3. **LeetCode 687. Longest Univalue Path（最长同值路径）**<<<<<<
    ...
    - 思路：递归法，计算每个节点的 “左同值链长度 + 右同值链长度”，取最大值；同值链长度为当前节点与子节点值相同时的长度 + 1。
        ...
        **先序遍历所有node，对于每个node再用后序遍历算相同的node数**
        ...
        class Solution {
        public:
            int maxlen = 0; // 初始值修正为0
            // 递归函数返回：以root为起点，向下延伸的同值路径的最长边数
            int dfs(TreeNode* root) {
                if (!root) return 0;
                
                int left = dfs(root->left);  // 左子树的最长同值边数
                int right = dfs(root->right); // 右子树的最长同值边数
                
                // 计算以当前root为中点的同值路径长度（边数）
                int cur_left = 0, cur_right = 0;
                if (root->left && root->left->val == root->val) {
                    cur_left = left + 1; // 包含root→left这条边
                }
                if (root->right && root->right->val == root->val) {
                    cur_right = right + 1; // 包含root→right这条边
                }
                
                maxlen = max(maxlen, cur_left + cur_right); // 无需加1！
                return max(cur_left, cur_right); // 返回单边最长边数
            }
...
4. **LeetCode 124. Binary Tree Maximum Path Sum（二叉树中的最大路径和）**(直径)
...
6. **LeetCode 333. Largest BST Subtree（最大 BST 子树）** <<<
    ...
    - 除了根节点，下面所有子树中，子树的节点个数最多的那个子树，返回子树node的个数
...
struct Info
{
    Info(bool b, int ma, int miv, int mav): bst(b), maxNodeCount(ma), minval(miv), maxval(mav) {}
    Info(): bst(true), maxNodeCount(0), minval(INT_MAX), maxval(INT_MIN) {}  // 修改1：空节点视为BST
...
class Solution {
public:
    Info order_search(TreeNode* root)
    {
        if (root == nullptr)
            return Info();  // 使用默认构造函数
        
        auto l = order_search(root->left);
        auto r = order_search(root->right);
        
        // 检查当前节点是否能与左右子树构成BST
        bool isBST = l.bst && r.bst && 
                     (root->val > l.maxval) &&  // 注意：对空子树，l.maxval=INT_MIN
                     (root->val < r.minval);    // 对空子树，r.minval=INT_MAX
        
        int curNodeSize = 0;
        if (isBST)
        {
            curNodeSize = l.maxNodeCount + r.maxNodeCount + 1;
        }
        else
        {
            curNodeSize = max(l.maxNodeCount, r.maxNodeCount);
        }
        
        // 更新最小值和最大值
        int retMin = min(root->val, min(l.minval, r.minval));
        int retMax = max(root->val, max(l.maxval, r.maxval));
        
        return Info(isBST, curNodeSize, retMin, retMax);
    }

    int largestBSTSubtree(TreeNode *root) {
        auto resInfo = order_search(root);
        return resInfo.maxNodeCount;
    }
};
```
