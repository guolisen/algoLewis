...
**IQR（Interquartile Range，四分位距）** 是一种用于识别数据集中异常值的统计方法，特别在数据清洗和探索性数据分析中广泛使用。

## **一、核心概念解析**

### 1. **四分位数基础**

- **Q1（第一四分位数）**：数据按升序排列后，位于25%位置的值
- **Q2（第二四分位数）**：中位数，位于50%位置的值
- **Q3（第三四分位数）**：位于75%位置的值
- **IQR = Q3 - Q1**：中间50%数据的分布范围
