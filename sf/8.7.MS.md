...
合并 k 个有序数组是经典的算法问题，有多种解决方案。我来为你详细解释并提供 Python 实现。

## 问题理解

我们有 k 个已经排序的数组，需要将它们合并成一个有序数组。

## 解决方案

### 方法 1: 使用最小堆（最优解）

**时间复杂度**: O(N log k)，其中 N 是所有元素的总数
**空间复杂度**: O(k)
...
def merge_k_sorted_arrays_min_heap(arrays: List[List[int]]) -> List[int]:
    """
    使用最小堆合并 k 个有序数组
    """
    if not arrays:
...
def merge_k_sorted_arrays_divide_conquer(arrays: List[List[int]]) -> List[int]:
    ...
    def merge_helper(arrays, left, right):
        """递归合并数组"""
        if left == right:
            return arrays[left]
        ...
        left_merged = merge_helper(arrays, left, mid)
        right_merged = merge_helper(arrays, mid + 1, right)
...
```
def merge_k_sorted_arrays_sequential(arrays: List[List[int]]) -> List[int]:
    """
    顺序合并数组（效率较低）
    """
    if not arrays:
    ...
    for i in range(1, len(arrays)):
        ...
        while p1 < len(result) and p2 < len(arrays[i]):
            if result[p1] <= arrays[i][p2]:
                new_result.append(result[p1])
                p1 += 1
            else:
                new_result.append(arrays[i][p2])
                p2 += 1
        
        new_result.extend(result[p1:])
        new_result.extend(arrays[i][p2:])
        result = new_result
...
```
def merge_k_sorted_arrays_brute_force(arrays: List[List[int]]) -> List[int]:
    """
    暴力解法：将所有元素收集起来然后排序
    """
    result = []
...
def test_merge_functions():
    ...
    functions = [
        ("Min Heap", merge_k_sorted_arrays_min_heap),
        ("Divide & Conquer", merge_k_sorted_arrays_divide_conquer),
        ("Sequential", merge_k_sorted_arrays_sequential),
        ("Brute Force", merge_k_sorted_arrays_brute_force)
    ]
    
    for i, arrays in enumerate(test_cases):
        print(f"\nTest Case {i + 1}: {arrays}")
...
| 方法     | 时间复杂度 | 空间复杂度 | 适用场景                   |
| :------- | :--------- | :--------- | :------------------------- |
| 最小堆   | O(N log k) | O(k)       | **最优解**，k 很大时表现好 |
| 分治法   | O(N log k) | O(N)       | 易于实现，递归方式         |
| 顺序合并 | O(kN)      | O(N)       | k 很小时可以考虑           |
| 直接排序 | O(N log N) | O(N)       | 最简单但效率最低           |
...
1. 堆中存储 `(值, 数组索引, 元素索引)` 三元组
2. 初始化时将所有数组的第一个元素入堆
3. 每次弹出最小值后，将同一数组的下一个元素入堆
4. 直到堆为空，完成合并

这种方法能够有效地利用各个数组的有序特性，确保每次都能取出全局最小值。
