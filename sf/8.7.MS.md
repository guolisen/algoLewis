...
public:
    string reverseWords(string s) {
        ...
        for (int start = 0; start < m; start++)
        {
            if (s[start] == ' ')
                continue;
            int curWrite = writeIdx;
            int end = start;
            while (end < m && s[end] != ' ')
            {
                s[curWrite++] = s[end++];
            }
            reverse(s.begin() + writeIdx, s.begin() + curWrite);
            s[curWrite++] = ' ';
            writeIdx = curWrite;
            start = end;
        }
...
int main() {
    ...
    deque<int> q; // 存储索引，而不是值
    
    // 求滑动窗口最小值
    for (int i = 1; i <= n; i++) {
        // 1. 先检查队头是否滑出窗口（优化：先处理窗口移动）
        if (!q.empty() && q.front() < i - k + 1) {
            q.pop_front();
        }
        
        // 2. 维护队列单调性：移除比新元素大的元素（保持递增）
