...
| **顶层const**     | 忽略                 | 保留                   |
| **引用**          | 丢弃引用，得到值类型 | 保留引用               |
| **解引用(\*ptr)** | 得到值类型           | 得到引用类型           |
| **函数调用**      | 实际执行函数         | 不执行，只分析返回类型 |
...
├── 是普通变量 → 用 auto
├── 需要引用 → 用 auto& 或 auto&&
└── 需要const引用 → 用 const auto&

需要写函数返回类型吗？
├── 简单返回值 → 用 auto
├── 需要返回引用 → 用 decltype(auto)
└── 需要完美转发 → 用 decltype(auto)

在模板或复杂表达式里吗？
├── 需要获取某个表达式的精确类型 → 用 decltype(expr)
└── 其他情况 → 用 auto
```
...
1. **普通变量声明，99%用 `auto`**
2. **需要引用时，用 `auto&` 或 `auto&&`**
3. **只有需要返回引用或完美转发时，才考虑 `decltype(auto)`**

```
std::vector<int> data = {1, 2, 3, 4, 5};

// 想写一个lambda返回vector的某个元素的引用
auto lambda_wrong = [&data](int idx) {
    return data[idx];  // 返回int，不是引用
};

auto lambda_right = [&data](int idx) -> decltype(auto) {
    return data[idx];  // 返回int&
};

int main() {
    lambda_right(0) = 10;  // ✅ 修改data[0]
    // lambda_wrong(0) = 10;  // ❌ 错误
    return 0;
}
```
...
含义：`a` 是一个指针，指向一个 `const int`（即整型常量）。
也就是说，**不能通过 `a` 来修改它所指向的那个整数的值**，但 `a` 本身可以改变指向另一个地址。
...
const int y = 20;
const int* a = &x;  // 正确
// *a = 30;         // 错误：不能通过 a 修改它指向的值
a = &y;             // 正确：可以让 a 指向另一个 const int
```
...
这个和 `const int* a` **完全一样**，只是 `const` 放在了 `int` 后面。
它依然表示：指针指向的是一个常量整型，不能通过 `a` 修改它指向的值，但 `a` 本身可以改指向。

cpp

```
int const* a;       // 和 const int* a 无区别
```
...
含义：`a` 是一个 **常量指针**（指针本身是常量），指向一个普通 `int`。
也就是说，`a` 一旦初始化后就不能再指向其他地址，但是可以通过 `a` 修改它所指向的整数的值。
...
- `const int* a` 读作：`a` is a pointer to const int
    即：指针指向常量数据，指针可变，数据不可变。
    其实这里 `const` 修饰的是 `*a`（指针解引用的内容），而不是 `a`。
- `int const* a` 同样读作：`a` is a pointer to const int（const 修饰的是 int）
- `int* const a` 读作：`a` is a const pointer to int
    `const` 修饰的是 `a`（指针变量本身），而不是 `*a`。

------

## 3. 组合情况

还有 `const int* const a`：
表示 `a` 是一个常量指针，指向一个常量整数。既不能改变 `a` 的指向，也不能通过 `a` 修改它指向的值。
...
| 声明类型             | 能否改变指针指向（`a = &b;`） | 能否通过 `*a = ...` 修改指向的值 |
| :------------------- | :---------------------------- | :------------------------------- |
| `const int* a`       | ✅ 可以                        | ❌ 不可以                         |
| `int const* a`       | ✅ 可以                        | ❌ 不可以                         |
| `int* const a`       | ❌ 不可以                      | ✅ 可以                           |
| `const int* const a` | ❌ 不可以                      | ❌ 不可以                         |
