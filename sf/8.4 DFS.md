...
class Solution:
    ...
    def dfs(self, board, x, y, tree, visit, res):
        if tree.word != "" and tree.word not in res:
        ...
        for i in range(4):
            ...
            if not self.isInBound(nx, ny, m, n) or visit[nx][ny] == True:
                continue
            if board[nx][ny] not in tree.children:
                continue
            visit[nx][ny] = True
            self.dfs(board, nx, ny, tree.children[board[nx][ny]], visit, res)
            visit[nx][ny] = False
    ...
    def word_search_i_i(self, board: List[List[str]], words: List[str]) -> List[str]:
        ...
        for i in range(m):
            for j in range(n):
                if not tree.children[board[i][j]]:
                    continue
                visit = [[False] * n for _ in range(m)]
                visit[i][j] = True
                self.dfs(board, i, j, tree.children[board[i][j]], visit, res)
        return res
```
