        ...
        while q:
            ...
            for _ in range(count):
                ...
                for i in range(4):
                    nx = cx + dx[i]
                    ny = cy + dy[i]
                    if not self.isInBounds(nx, ny, m, n) or grid[nx][ny] != 1:
                        continue
                    grid[nx][ny] = 2
                    q.append((nx, ny))
            step += 1
...
```
class Solution:
    def dfs(self, k, target, pre, sum, curlist, res):
        if sum > target or len(curlist) > k:
            return
        if sum == target and len(curlist) == k:
            res.append(curlist[:])
        for i in range(pre, 10):
            curlist.append(i)
            self.dfs(k, target, i + 1, sum + i, curlist, res)
            curlist.pop()
...
如果只是相邻元素去重的话，像这个例子[1,2,3,1,1,1]就会出现【1,1】【1,1,1】重复



核心原因：Combination Sum II 的「重复」是「索引依赖」的，而 `h=set()` 是「值依赖」的

Combination Sum II 的题目规则：

1. 原数组可能有重复元素（如 `[10,1,2,7,6,1,5]`）；
2. 每个元素**只能用一次**（索引不能重复）；
3. 结果不能有重复组合（如 `[1,7]` 只能出现一次）。
...
class Solution:
    def isInBounds(self, x, y, m ,n):
        return 0 <= x < m and 0 <= y < n 
    def dfs(self, matrix, x, y, length, visit):
        m = len(matrix)
        n = len(matrix[0])
        dx = [0,0,-1,1]
        dy = [-1,1,0,0]
        ...
        for i in range(4):
            nx = x + dx[i]
            ny = y + dy[i]
            if not self.isInBounds(nx, ny, m, n) or matrix[nx][ny] <= matrix[x][y]:
                continue 
            if visit[nx][ny]:
                continue
            visit[nx][ny] = True
            res = max(res, self.dfs(matrix, nx, ny, length + 1, visit))
            visit[nx][ny] = False
        return res
    ...
    def longestIncreasingPath(self, matrix: List[List[int]]) -> int:
        ...
        for i in range(m):
            for j in range(n):
                ...
                visit = [[False] * n for _ in range(m)]
                visit[i][j] = True 
                hs[(i, j)] = self.dfs(matrix, i, j, 0, visit)
                res = max(res, hs[(i, j)])
        return res + 1
```
    ...
    def dfs(self, matrix, x, y, visit):
        ...
        for i in range(4):
            ...
            if visit[nx][ny]:
                continue
            visit[nx][ny] = True
            res = max(res, 1 + self.dfs(matrix, nx, ny, visit))
            visit[nx][ny] = False
        self.mem[(x, y)] = res
        return res
    ...
    def longestIncreasingPath(self, matrix: List[List[int]]) -> int:
        ...
        for i in range(m):
            for j in range(n):
                visit = [[False] * n for _ in range(m)]
                visit[i][j] = True 
                res = max(res, self.dfs(matrix, i, j, visit))
        return res + 1
```
    ...
    def dfs(self, node, hs):
        ...
        for n in node.neighbors:
            if n in hs.keys():
                ...
                ne = self.dfs(n, hs)
                hs[node].neighbors.append(ne)
        return hs[node]
    def cloneGraph(self, node: Optional['Node']) -> Optional['Node']:
        hs = dict()
        return self.dfs(node, hs)
```
